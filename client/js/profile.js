        ...
        $.ajax({
          ...
          success: function (res) {
            if (
              ...
              res.comments == "" ||
              res.comments == null ||
              res.comments.length < loadAmounts
            ) {
          ...
          error: function () {
            $("#comments-container").append(
              '<div style="font-size: 25px" class="comments-empty res-empty">No Comments to Display</div>'
            );
          },
        }).done(function () {
...
function loadJoinedForums(forums, append) {
  ...
  for (let forum in forums) {
    ...
    let html =
      '<div class="profile-forum" ><img class="profile-forum-icon" src="' +
      forums[forum].icon +
      '" ><div class="profile-forum-title" title="Owned By ' +
      forums[forum].owner +
      '" ><a href="/forum.php?fquery=' +
      forums[forum].name +
      '" >' +
      forums[forum].name +
      '</a><div class="profile-forum-date" >' +
      forums[forum].date +
      '</div></div><div class="profile-forum-desc" >' +
      description +
      "</div></div>";
    switch (append) {
      case true:
...
function createForum() {
  ...
  if (forumName.length > 15) {
    ...
    popUp(
      "clientm-fail",
      "Forum Description Must be Under 300 Characters",
      null
    );
  } else if (forumDescription.length == 0) {
    popUp(
      "clientm-fail",
      "Forum Description Must be More than 0 Characters",
      null
    );
  } else if (forumIconURL.length > 150) {
    popUp("clientm-fail", "Forum Icon URL Must be Under 150 Characters", null);
