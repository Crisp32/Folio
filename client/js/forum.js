...
function triggerOnForumLoad() {
  ...
  $(document).on(
    "click",
    "#members-modal .view-member, .banned-members-container .view-member",
    function (e) {
  ...
  $(document).on("click", ".member-action", function (e) {
    ...
    switch (action) {
      case "kick":
        warning = "Are you sure you want to Kick " + profile + "?";
        delElement = true;
        break;
      case "ban":
        warning = "Are you sure you want to Ban " + profile + "?";
        delElement = true;
        break;
      case "promote":
        warning = "Are you sure you want to Promote " + profile + "?";
        break;
      case "demote":
        warning = "Are you sure you want to Demote " + profile + "?";
        break;
      case "unban":
        warning = "Are you sure you want to Unban " + profile + "?";
        delElement = true;
        break;
    }
  ...
  $(document).on("click", "#confirm-member-action", function (e) {
    if (confirm.delElement) {
      ...
      if ($(".banned-members-container").length == 1) {
        $(".bans-empty").remove();
        $(".banned-members-container").append(
          '<div class="bans-empty res-empty" style="font-size: 25px; display: block;">No Banned Members</div>'
        );
      }
    }
          ...
          success: function (res) {
            if (res.success) {
              ...
              if (
                ...
                res.posts == [] ||
                res.posts == "" ||
                res.posts.length < postLoadAmounts
              ) {
                loadedAllPosts = true;
              }
...
function initForumButtons() {
  ...
  $(document).on("click", ".forum-post-container .delete-post", function (e) {
    ...
    deletePost.pid = $(deletePost.element)
      .parent()
      .siblings(".forum-post-voting")
      .attr("data-pid");
      ...
      success: function (res) {
        if (res.success) {
          ...
          if ($("#forum-posts-container").children().length == 0) {
            $("#forum-posts-container").append(
              '<div class="res-empty posts-empty profile-section" >No Posts to Display</div>'
            );
          }
  ...
  $(document).on("click", ".show-post-comments", function (e) {
    ...
    let commentsElement = $(element)
      .parent()
      .parent()
      .siblings(".forum-post-comments");
          ...
          if (res.success) {
            ...
            if (
              ...
              loadPostComments(
                res.comments,
                true,
                $(commentsElement).find(".forum-post-comments-container"),
                true
              );
            }
          } else {
        ...
        success: function (res) {
          if (res.success) {
            ...
            loadPostComments(
              res.comment,
              false,
              $(element).parent().siblings(".forum-post-comments-container"),
              false
            );
          } else {
            popUp("clientm-fail", res.message, null);
  ...
  $(document).on("click", ".del-comment", function (e) {
    ...
    let emptyElement = $(element)
      ...
      .parent()
      .find(".res-empty");
    let commentContainer = $(emptyElement).siblings(
  ...
  $(document).on("click", ".load-more-comments", function (e) {
    let element = $(this);
    let votingElement = $(element)
      .parent()
      .parent()
      .siblings(".forum-post-container")
      .find(".forum-post-voting");
    let postId = $(votingElement).attr("data-pid");
    let commentsElement = $(element)
      .parent()
      .siblings(".forum-post-comments-container");
...
function loadMembers(members) {
  ...
  for (let member in members) {
    let html =
      '<div class="profile-forum" data-profile="' +
      members[member].username +
      '" ><img class="member-img" src="' +
      members[member].image +
      '" ><div class="forum-member-name" >' +
      members[member].username +
      '</div><br /><div class="member-options" ><button class="view-member member-default-option" >View</button></div></div>';
    ...
    if (memberData.removable) {
      ...
      $(options).append(
        '<button class="member-action member-default-option member-option-red" data-action="kick" >Kick</button>'
      );
      $(options).append(
        '<button class="member-action member-default-option member-option-red" data-action="ban" >Ban</button>'
      );

      if (memberData.promotable) {
        $(options).append(
          '<button class="member-action member-default-option member-option-green" data-action="promote" >Promote</button>'
        );
      }

      if (memberData.demotable) {
        $(options).append(
          '<button class="member-action member-default-option member-option-red" data-action="demote" >Demote</button>'
        );
      }
    }
  }
...
function openForumSettings() {
  ...
  $(".create-forum-modal .forum-icon-sel").attr(
    "src",
    $(".forum-img#profile-img").attr("src")
  );
    ...
    for (let ban in bans) {
      ...
      $(container).append(
        '<div class="profile-forum" data-profile="' +
          ...
          '" ><div class="forum-member-name" >' +
          bannedMember.username +
          '</div><br /><div class="member-options" ><button class="view-member member-default-option" >View</button><button class="member-action member-default-option member-option-green" data-action="unban" >Unban</button></div></div>'
      );
    }
  }
}
...
function saveForum() {
  ...
  if (iconURL.length > 150) {
    popUp(
      "clientm-fail",
      "Forum Icon URL Must be 150 Characters or Less",
      null
    );
  } else if (description.length > 300) {
    popUp(
      "clientm-fail",
      "Forum Description Must be 300 Characters or Less",
      null
    );
  } else if (description.length == 0) {
    popUp(
      "clientm-fail",
      "Forum Description Must be Greater than 0 Characters",
      null
    );
  } else {
...
function loadForumPosts(posts, append) {
  ...
  if (posts.length > 0) {
    for (let post in posts) {
      ...
      let downvoteClasses = "";
      let actionButtons =
        '<button class="show-post-comments forum-post-action member-default-option" >View Comments</button>';
      ...
      if (postObject.canEdit) {
        actionButtons +=
          '<button class="delete-post forum-post-action member-default-option member-option-red" >Delete</button>';
      }
      ...
      let html =
        '<div class="forum-post-wrapper" ><div class="profile-section forum-post-container" ><h2 class="section-title" >' +
        postTitle +
        '</h2><br /><div class="forum-post-info" >Posted ' +
        postObject.date +
        ' by <a style="color: ' +
        posterNameColour +
        '" href="' +
        ...
        "</a> " +
        forumNameHtml +
        '</div><div class="forum-post-body" >' +
        postBody +
        '</div><div class="forum-post-voting" data-comments="' +
        postObject.comments +
        '" data-pid="' +
        postObject.pid +
        '" ><button title="Upvote" class="upvote vote' +
        upvoteClasses +
        '" ><img src="/images/other/voteIcon.svg" ></button><button title="Downvote" class="downvote vote' +
        downvoteClasses +
        '" ><img src="/images/other/voteIcon.svg" ></button><div class="forum-post-votes" style="color: ' +
        voteCountColour +
        '" >' +
        postObject.voteCount +
        '</div></div><div class="forum-post-actions" >' +
        actionButtons +
        '</div></div><br /><div class="profile-section forum-post-comments" ><div class="add-post-comment-div" ><input class="add-comment post-forum-comment" placeholder="Comment" /><button class="post-forum-comment add-comment-btn" >Post</button></div><div class="res-empty post-comments-empty" >No Comments to Display</div><div class="forum-post-comments-container" ></div></div></div></div>';
...
function loadPostComments(
  ...
  for (let comment in commentsJSON) {
    ...
    let commentHTML =
      '<div class="comment-full" ><div class="comment" ><div class="commenter-name" ><a style="color: ' +
      nameColour +
      '" href="' +
      userLink +
      '" >' +
      ...
      commentsJSON[comment].date +
      '</div></div><div class="likes-container" ><button class="likes-icon' +
      imgLikedClass +
      '" ><img title="I Like this Comment" src="/images/other/like-icon.svg" ></button><div class="likes-count' +
      likedClass +
      '" >' +
      commentsJSON[comment].likes +
      '</div><br /><div name="' +
      commentsJSON[comment].cid +
      '" class="del-comment delete-comment noselect" style="display: ' +
      commentsJSON[comment].delDisplay +
      '" >Delete</div></div><div class="comment-content" >' +
      commentBody +
      '</div></div><div class="add-reply" ><input class="add-comment" placeholder="Reply" /><button class="add-comment-btn post-reply-btn" >Post</button></div><div class="replies-container" >' +
      replyHTML +
      "</div></div>";
  ...
  if (commentsJSON.length > commentLoadAmounts - 1 && showLoadCommentsButton) {
    $(element)
      .parent()
      .append(
        '<div style="text-align: center" ><button class="load-more-comments" >Load More Comments</button></div>'
      );
  }
}
...
function applySort() {
  ...
  if (
    sort !== sortMethod &&
    !showEmptyMsg &&
    $("#forum-posts-container").children().length > 1
  ) {
    sort = sortMethod;
    popUp("clientm-fail", "Loading...", null);
