...
function renderUserList(usersJSON) {
  for (let u = 0; u < usersJSON.length; u++) {
    ...
    let html =
      '<div class="home-list-item" ><div class="item-num" >' +
      (u + 1) +
      ' <div class="bullet-point" style="color: #0cdc00" >&#8226;</div></div><img class="item-icon" src="' +
      user.image +
      '" ><div class="item-title-sub" ><div class="item-title" ><a href="/profile.php?uquery=' +
      user.username +
      '" >' +
      user.username +
...
function renderForumList(forumsJSON) {
  for (let f = 0; f < forumsJSON.length; f++) {
    ...
    let html =
      ...
      ' <div class="bullet-point" style="color: #0cdc00" >&#8226;</div></div><img class="item-icon" src="' +
      forum.icon +
      '" ><div class="item-title-sub" ><div class="item-title" ><a href="/forum.php?fquery=' +
      forum.name +
      '" >' +
      forum.name +
      '</a></div><div class="item-sub" >' +
